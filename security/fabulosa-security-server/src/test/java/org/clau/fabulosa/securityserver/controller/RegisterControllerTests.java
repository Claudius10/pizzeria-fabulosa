...
import org.clau.fabulosa.securityserver.TestHelperService;
...
public class RegisterControllerTests {
   ...
   private final String path = ApiRoutes.API + ApiRoutes.V1 + ApiRoutes.REGISTER;
   ...
   void givenRegisterApiCall_thenRegisterUser() throws Exception {
	  ...
	  mockMvc.perform(post(path + ApiRoutes.USER_BASE)
			.with(csrf())
   ...
   void givenRegisterApiCall_whenAccountWithEmailAlreadyExists_thenDontAllowRegister() throws Exception {
	  ...
	  MockHttpServletResponse response = mockMvc.perform(post(path + ApiRoutes.USER_BASE)
			.with(csrf())
			.contentType(MediaType.APPLICATION_JSON)
			.content(objectMapper.writeValueAsString(registerDTO)))
	  ...
	  assertThat(responseObj.getApiError().getMessage()).isEqualTo(ApiResponseMessages.USER_EMAIL_ALREADY_EXISTS);
   }
