...
import org.clau.pizzeriasecurityserver.MyTestcontainersConfiguration;
...
public class AnonUserControllerTests {
   ...
   private final String path = Route.API + Route.V1 + Route.PUBLIC + Route.USER_BASE;
   ...
   void givenRegisterApiCall_thenRegisterUser() throws Exception {
	  ...
	  mockMvc.perform(post(path + Route.REGISTER)
			.with(csrf())
   ...
   void givenRegisterApiCall_whenAccountWithEmailAlreadyExists_thenDontAllowRegister() throws Exception {
	  ...
	  MockHttpServletResponse response = mockMvc.perform(post(path + Route.REGISTER)
			.with(csrf())
			.contentType(MediaType.APPLICATION_JSON)
			.content(objectMapper.writeValueAsString(registerDTO)))
