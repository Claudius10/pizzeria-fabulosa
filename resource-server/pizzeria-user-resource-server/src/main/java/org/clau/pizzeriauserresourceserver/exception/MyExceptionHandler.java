...
import org.clau.pizzeriauserresourceserver.service.ErrorService;
...
public class MyExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<ResponseDTO> authenticationException(AuthenticationException ex, WebRequest request) {
		...
		String path = extractPath(request);
		ResponseDTO response = handleAuthenticationException(ex, path);
		ExceptionLogger.log(ex, log, response);
		return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(response);
	}
	...
	private ResponseDTO handleAuthenticationException(AuthenticationException ex, String path) {
