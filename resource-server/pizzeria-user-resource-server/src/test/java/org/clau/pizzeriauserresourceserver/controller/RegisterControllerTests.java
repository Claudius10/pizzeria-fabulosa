...
public class RegisterControllerTests {
   ...
   private final String path = Route.API + Route.V1 + Route.REGISTER;
   ...
   void givenRegisterApiCall_thenRegisterUser() throws Exception {
	  ...
	  mockMvc.perform(post(path + Route.USER_BASE)
			.with(csrf())
   ...
   void givenRegisterApiCall_whenAccountWithEmailAlreadyExists_thenDontAllowRegister() throws Exception {
	  ...
	  MockHttpServletResponse response = mockMvc.perform(post(path + Route.USER_BASE)
			.with(csrf())
			.contentType(MediaType.APPLICATION_JSON)
			.content(objectMapper.writeValueAsString(registerDTO)))
